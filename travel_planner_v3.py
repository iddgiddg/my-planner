...
# 1. 웹페이지 기본 설정
st.set_page_config(page_title="나만의 사진 여행 플래너 ✈️", page_icon="📸", layout="wide")

# ⭐ [여기에 주소를 입력하세요!] 
# 1단계에서 복사한 본인의 구글 파이어베이스 Realtime Database 주소를 따옴표 안에 넣으세요.
# 주소 끝에 슬래시(/)가 빠져있다면 꼭 붙여주셔야 합니다!
...
# --- 🔐 인증 시스템 화면 (로그인 / 회원가입) ---
def show_auth_page():
    st.markdown("<h2 style='text-align: center; margin-bottom: 20px;'>📸 영구 저장형 사진 여행 플래너</h2>", unsafe_allow_html=True)
    col1, col2, col3 = st.columns([1, 1.2, 1])
    with col2:
        mode = st.radio("원하는 작업을 선택하세요", ["로그인", "회원가입"], index=0 if st.session_state.auth_mode == "로그인" else 1, horizontal=True)
        st.markdown("---")
        ...
        if st.session_state.auth_mode == "로그인":
            ...
            with st.form(key="join_form"):
                ...
                if submit_join:
                    if not new_id or not new_pw: st.error("❌ 모든 칸을 입력해 주세요.")
                    elif db_load(f"users/{new_id}") is not None: st.error("❌ 이미 존재하는 아이디입니다.")
                    elif new_pw != confirm_pw: st.error("❌ 비밀번호가 일치하지 않습니다.")
                    else:
...
if not st.session_state.logged_in:
    ...
    st.markdown("---")
    st.caption("🗺️ 사진 여행 플래너 - Google Firebase 실시간 클라우드 DB 연동 완료.")